...
// on Pro plan) so weekly sheets don't time out. Hobby plan silently caps this.
...
// SPEED: fires 3 PARALLEL LLM calls (one recipe style each) instead of one big
...
- 5-9 numbered cooking steps, each 10-25 words, mention temperatures, times, and pans/tools.
- Recipe must be authentic and realistic — exactly how the trusted source would make it.
...
Output STRICT JSON. No trailing commas.`
  ...
  const body = {
    ...
    messages: [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: `Find the best "${style}" recipe from top cooking websites for: "${query}". Return JSON.` }
    ],
    temperature: 0.4,
    response_format: { type: 'json_object' },
  }
  ...
  try {
    ...
    if (path === 'scan' || path === 'scan-receipt' || path === 'parse-voice' || path === 'recipe-instructions' || path === 'identify-product' || path === 'recipe/generate' || path === 'recipe/web-search' || path === 'haccp/scan-temperatures') {
      ...
      if (path === 'recipe/web-search') {
        ...
        const opts = {
          query,
          servings: Math.max(1, Math.min(20, Number(body.servings) || 1)),
          dietary: Array.isArray(body.dietary) ? body.dietary.slice(0, 8) : [],
        }
    ...
    if (segs[0] === 'staff' && segs[1]) {
      ...
      const next = list.map(e => {
        if (String(e?.name || '').toLowerCase() === target) {
          found = true
          updated = { ...e }
          if (body.role !== undefined) updated.role = body.role === 'manager' ? 'manager' : 'staff'
      ...
      if (uErr) throw uErr
      return json({ ok: true, name: decodeURIComponent(segs[1]), role: updated.role || 'staff', perms: updated.perms || [] })
    }
