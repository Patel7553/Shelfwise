 ...
 */
export function signChefToken(kitchenId, person = null) {
  const secret = process.env.SHELFWISE_JWT_SECRET
  if (!secret) throw new Error('SHELFWISE_JWT_SECRET env var missing')
  const payload = { kitchen_id: kitchenId, role: 'chef' }
  // Staff-PIN logins embed WHO this token belongs to — attribution can't be
  // spoofed by editing the x-person-name header.
  if (person) payload.person = String(person).slice(0, 40)
...
export async function getAuthContext(request) {
  ...
  if (chef && chef.kitchen_id) {
    return { authed: true, role: 'chef', kitchenId: chef.kitchen_id, kitchen: null, userEmail: null, isAdmin: false, person: chef.person || null }
  }
