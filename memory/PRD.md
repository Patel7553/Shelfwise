...
- **Prices ON the Stock Levels list + 2 push alerts**: (1) stock-levels.jsx now fetches every connected supplier's catalog on load; each row shows the catalog price (bold £X.XX), amber "No price set" when £0, em-dash when not in catalog, ellipsis while loading. Verified via screenshot. (2) PRICE CHANGE ALERT: PUT /api/supplier/products/:id captures old price; if changed, pushes to connected kitchens that stock the item (inventory name fuzzy) or bought it (order history productId): "Price change at {sup}: {item} £old -> £new". Triggers when supplier edits price in their dashboard (couldn't self-test: needs Supabase supplier session; logic code-reviewed). (3) LOW STOCK ALERT: usage/apply + PUT products/:id push "Low stock: {name} down to {qty}" when qty crosses reorder_point (default 2); usage/apply also pushes "Out of stock" when fully used/removed. Live-tested: 5 -> 1.5 crossing fired push (no errors), test item cleaned up.
- **Shopping-cart checkout flow (rebuilt)**: NEW lib/cart.js (localStorage sw_cart_v1, sw-cart-changed event; stores supplierId/productId/qty only — prices always live). NEW components/shelfwise/cart.jsx CartView: grouped by supplier, live catalog prices, qty steppers, remove, per-group subtotal + min-order warning, search across ALL connected catalogs to add more, single CHECKOUT button = the only order-sending action (one POST /api/kitchen/orders per supplier, clears sent groups, success panel with refs). stock-levels.jsx: "Review orders" replaced by "Add to Cart" (instant matching via preloaded catalogs, toast with View-cart action, warns unmatched); review dialog removed. page.js: persistent header cart icon w/ count badge (desktop+mobile, kitchen+orders perm only), view 'cart'. E2E tested: add 2 -> badge 2 -> cart -> search-add milk -> qty bump -> £123.95 -> Checkout -> real order ORD-96B54B £148.74 (then cancelled) -> badge cleared. NOTE: user set real prices on PATEL FOOD catalog overnight. Test note: install-prompt dismiss key = shelfwise-install-dismissed-at.
- **Bidfood-style B2B checkout (rebuilt cart.jsx)**: 5-stage flow, NO payment step ever. (1) Header cart icon now shows count badge + running subtotal (price snapshot stored in cart entries, display-only). (2) Basket: itemized table Description/Size-Unit/Qty(+/-)/Price/Subtotal + VAT + total per supplier, catalog search-add, sticky bar with subtotal + Checkout. (3) Step 1 Order Details: account (kitchenName+person props from page.js: settings.kitchenName, me?.userEmail), auto order date, REQUIRED editable Order Reference (default PO-YYYYMMDD-HHMM), optional promo code. (4) Step 2 Delivery: address textarea (localStorage sw_delivery_address_v1, saved for next time), delivery date picker (min today, default tomorrow), itemized review. (5) Step 3 Review & Submit: per-supplier VAT tables + dark grand summary (line items/qty total/subtotal/VAT/grand total) + Submit -> POST per supplier with notes 'Order ref: X · Code: Y · Deliver to: Z' + requestedDeliveryDate. (6) Confirmation receipt: placed-by/email/order date/ref/delivery date+address, supplier name + account clientCode + API ref, itemized tables, Home/Email(mailto)/Print(window.print) buttons; failed groups stay in cart. E2E tested full flow; test order ORD-FB5808 £147.36 verified (notes + delivery date persisted) then cancelled.
- **Unified Trash / Recently Deleted (app-wide soft delete)**: every DELETE (products, recipes, receipts, supplier contacts, rota shifts, waste, HACCP temps/cleaning-log/delivery-checks, supplier catalog products) snapshots the full row via moveToTrash() into storage receipts/trash-bins/{kid}.json BEFORE deleting. 30-day auto-expiry (pruned on read/write, cap 300). Receipt files kept while in trash; removed on permanent purge. Endpoints: GET /api/trash (added to ownerOrChef GET list), POST /api/trash/restore {id} (upsert row back, kitchen/supplier ownership check, table whitelist), DELETE /api/trash/{id}. CRITICAL FIX: Supabase storage eventual-consistency solved with in-memory write-through cache (globalThis.__swTrashCache; cache-first reads, cacheControl:'0' uploads) — single-process app so cache is authoritative. UI: TrashCard in Settings -> Login & Alerts tab (collapsible, icons per type, deleted when/by whom, Restore + delete-forever w/ confirm, 30-day notes). Backend 7/7 tests passed; UI restore verified (note: Playwright get_by_role name matching needs exact=True — header contains word 'restore').
- **"/api/version" no-store hardening (update-detection fix)**: user reported "saved to GitHub but no update in app". Explained: Save-to-GitHub only saves code; app updates require a new DEPLOYMENT (Emergent Deploy/Redeploy or Vercel building the commit). Real fix applied: /api/version now sends Cache-Control no-store (+CDN-Cache-Control, Vercel-CDN-Cache-Control) so proxies can never serve a stale version number to installed PWAs (which would block the self-update reload). Tested 5/5 by backend agent incl. regression (products, trash).
- **Trash discoverability + menu cleanup (user complaint)**: (1) TrashCard moved out of Login & Alerts into a NEW dedicated "Trash" tab in Settings dialog (settings-auth.jsx tabs array, tab === 'trash', TrashCard defaultOpen prop). (2) Voice and Invoice tiles REMOVED from dashboard Add Products menu (grid-cols-2: Snap Label + Manual under the Scan Barcode hero). Invoice scanning still available via Receipts tab. Verified via screenshots. IMPORTANT: user must REDEPLOY for these to reach production (kitchen-stock-39.emergent.host).
